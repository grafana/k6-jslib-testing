...
import { failTest, passTest, createMockAssertFn } from "./testing.js";
...
function runTest(testCase) {
    ...
    if (mockAssertFn.calls[0].condition !== testCase.expectedCondition) {
        failTest(testCase.name, `expected assertFn condition to be ${testCase.expectedCondition}`);
    }

    if (mockAssertFn.calls[0].soft !== false) {
        failTest(testCase.name, "expected assertFn to be called with soft === false");
    }
...
function testToBeInstanceOf() {
    ...
    if (mockAssertFn.calls[0].soft !== false) {
        failTest("toBeInstanceOf", "expected assertFn to be called with soft === false");
    }
