...
Deno.test("assert default to hard assertions", () => {
    try {
        ...
        assert(e instanceof AbortedTestError, "assert did not throw AbortedTestError");
        assert(e.message === "Test aborted: false is false", "assert did not throw AbortedTestError with correct message");
    }
});
...
Deno.test("assert soft mode", async (t) => {
    ...
    await t.step("false condition should throw AssertionFailedError", () => {
        try {
            ...
            assert(e instanceof AssertionFailedError, "assert did not throw AssertionFailedError");
            assert(e.message === "false is false", "assert did not throw AssertionFailedError with correct message");
        }
    });
});
